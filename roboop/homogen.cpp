	...
	//!  @brief Euler ZyZ rotation
{
	...
	rot(1,1) = c1*ca*c2-s1*s2;
	rot(1,2) = -c1*ca*s2-s1*c2;
	rot(1,3) = c1*sa;
	rot(2,1) = s1*ca*c2 + c1*s2;
	rot(2,2) = -s1*ca*s2 + c1*c2;
	rot(2,3) = s1*sa;
	rot(3,1) = -sa*c2;
	rot(3,2) = sa*s2;
	rot(3,3) = ca;
	...
	//!  @brief Obtain Roll, Pitch and Yaw from a rotation matrix.
{
	ColumnVector  a(3);
	double sp,cp;
	if ( fabs( R(1,3) ) < EPSLION && fabs(R(2,3)) < EPSLION )
	{
		a(1) = 0;
		sp = 0;
		cp = 1;
		a(2) = atan2(cp*R(1,3) + sp*R(2,3), R(3,3));
		a(3) = atan2(-sp * R(1,1) + cp * R(2,1), -sp*R(1,2) + cp*R(2,2));
	}
	else
	{
		a(1) = atan2(R(2,3), R(1,3)); // phi
		sp = sin(a(1));
		cp = cos(a(1));
		a(2) = atan2(cp*R(1,3) + sp*R(2,3), R(3,3)); // theta
		a(3) = atan2(-sp * R(1,1) + cp * R(2,1), -sp*R(1,2) + cp*R(2,2)); // psi
	}

	a.Release(); return a;
}
ReturnMatrix columnvector_unitize( const ColumnVector & R )
{
	ColumnVector a(3);
	a = R;
