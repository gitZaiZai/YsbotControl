...
void get_x_rotation_matrix(double theta, Matrix& m)
{
	...
	{
		Matrix rx = rotx(theta);
		m = rx.SubMatrix(1,3,1,3);
	}
}
...
void get_y_rotation_matrix(double theta, Matrix& m)
{
	if( m.Nrows() == 3 && m.Ncols() == 3 )
	{
		Matrix ry = roty(theta);
		m = ry.SubMatrix(1,3,1,3);
 	}
}
...
void get_z_rotation_matrix(double theta, Matrix& m)
{
	if( m.Nrows() == 3 && m.Ncols() == 3 )
	{
		Matrix rz = rotz(theta);
		m = rz.SubMatrix(1,3,1,3);
 	}
}

void vector_rotate(const double* v,    // [in]  «Ý
	const double* base, // [in]  
	double theta,       // [in]  
	const Matrix& prev, // [in]  
	const Matrix& post, // [in]  
	double* result)     // [out] 
{

	Matrix  vect_in(3, 1), vect_out(3, 3);
	vect_in(1, 1) = v[0];
	vect_in(2, 1) = v[1];
	vect_in(3, 1) = v[2];

	if( fabs(base[1]) < EPSLION && fabs(base[2]) < EPSLION ) // X ¶b
	{
		get_x_rotation_matrix(((base[0] > 0) ? 1: -1) * theta, vect_out);
		vect_out *= vect_in;
	}

	else if( fabs(base[2]) < EPSLION && fabs(base[0]) < EPSLION ) // Y ¶b
	{
		get_y_rotation_matrix(((base[1] > 0) ? 1: -1) * theta, vect_out);
		vect_out *= vect_in;
	}

	else if( fabs(base[0]) < EPSLION && fabs(base[1]) < EPSLION ) // Z ¶b
	{
		get_z_rotation_matrix(((base[2] > 0) ? 1: -1) * theta, vect_out);
		vect_out *= vect_in;
	}
	...
	result[0] = vect_out(1, 1);
	result[1] = vect_out(2, 1);
	result[2] = vect_out(3, 1);
}
...
void tr2orient(const TRANSFORMATION_RULES& tr, ORIENTATION* orient)
{
	...
	{
		orient->rx = atan2(  tr.V3, tr.W3 );
		orient->ry = atan2( -tr.U3, sqrt(tr.U1*tr.U1 + tr.U2*tr.U2) );
		orient->rz = atan2( tr.U2, tr.U1 );
	}
