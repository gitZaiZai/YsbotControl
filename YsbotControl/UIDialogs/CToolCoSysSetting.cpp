...
BEGIN_MESSAGE_MAP(CToolCoSysSetting, CGroupFrame)
  
END_MESSAGE_MAP()

CToolCoSysSetting::CToolCoSysSetting() 
{
...
BOOL CToolCoSysSetting::Create(CString caption,DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	...
	LPCTSTR	 strUnitLabel[] = { _T("mm"),  _T("mm"), _T("mm"),  _T("¡ã"), _T("¡ã"), _T("¡ã") };
...
void CToolCoSysSetting::SetData(double* JArray,unsigned int JNum)
{
	double joint[8];
	CString Jstr;
...
void CToolCoSysSetting::GetData(double* JArray,unsigned int JNum)
{
	double joint[8];
	CString Jstr;
	for (unsigned int i=0;i<JNum;i++)
...
void CToolCoSysSetting::SetReadOnly(BOOL bReadOnly)
{
...
void CToolCoSysSetting::clear_show()
{
	for (unsigned int i=0;i<6;i++)
	{
