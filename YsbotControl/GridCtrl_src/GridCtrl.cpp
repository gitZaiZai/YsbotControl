...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_crHighLightBackClr  = RGB(88,148,24);
	m_crHighLightTextClr  = RGB(0,0,0);       // liurundan
	...
	m_hBmpMinus     = NULL;		
	m_ScrollBarWnd  = NULL;
	m_bShowHorzSrollBar = TRUE;
	m_bShowVertSrollBar = TRUE;
	
	m_lastDownScrollPos = 0;
	m_bShowVertSrollBarWindow = FALSE;
	m_bShowImageCombobox = TRUE;
}
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle,
	HBITMAP hBmpScroolBar,	HBITMAP hBmpPlus, HBITMAP hBmpMinus, SkinWindowKind nWndKind, int baroffset )
{
...
// 	ModifyStyleEx(WS_EX_CLIENTEDGE,0,SWP_DRAWFRAME);
	...
	if ( hBmpMinus )
 		m_hBmpMinus = hBmpMinus;

	if ( hBmpScroolBar )
	{
		m_hBmpScrollBar = hBmpScroolBar;
		m_ScrollBarWnd = SkinWndScroll(this,m_hBmpScrollBar,baroffset,nWndKind);
  	}
...
LRESULT CGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage) const
{
	...
	if ( m_ScrollBarWnd )
	{
		return SendMessage(WM_NOTIFY, nmgv.hdr.idFrom, (LPARAM)&nmgv); // liurundan
	}
	else
	{
		...
		{
			int res = pOwner->SendMessage(WM_NOTIFY, nmgv.hdr.idFrom, (LPARAM)&nmgv);
			return res;
  		}
		else
			return 0;
...
//}}AFX_MSG_MAP
	...
	ON_NOTIFY(GVN_BEGINLABELEDIT, IDC_INPLACE_CONTROL, OnBeginInPlaceEdit)
END_MESSAGE_MAP()
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	DWORD oldData = GetItemData(pgvItem->row, pgvItem->col);
	if ( m_bShowImageCombobox && ( pgvItem->mask & GVIF_IMAGE) )  // liurundan
	{
		SetItemData(pgvItem->row, pgvItem->col,pgvItem->iImage);
		DWORD dwData = (DWORD)(pgvItem->iImage);
		int vset = HIBYTE(dwData); 	
 		SetItemImage(pgvItem->row, pgvItem->col, vset );
 	}

   // OnEndEditCell(pgvItem->row, pgvItem->col, pgvItem->strText);
	OnEndEditCellInPlace(pgvItem->row, pgvItem->col, pgvItem->strText,oldData); // liurundan
...
// liurundan add.
void CGridCtrl::OnBeginInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	GV_DISPINFO *pgvDispInfo = (GV_DISPINFO *)pNMHDR;
	GV_ITEM     *pgvItem = &pgvDispInfo->item;
	...
	SendMessageToParent(pgvItem->row, pgvItem->col, GVN_BEGINLABELEDIT);
}
...
// 	printf("scrollpos = %d npos = %d m_nVScrollMax = %d\n",scrollPos,nPos, m_nVScrollMax);
    ...
    case SB_LINEDOWN:
        if (scrollPos < m_nVScrollMax)
        {
			...
			int yScroll = GetRowHeight(idTopLeft.row);
 			if ( scrollPos + yScroll > m_lastDownScrollPos )
			{
				SetScrollPos32(SB_VERT, scrollPos + yScroll);
				m_lastDownScrollPos = scrollPos + yScroll;
    ...
    case SB_LINEUP:
        if (scrollPos > 0 && idTopLeft.row > GetFixedRowCount(TRUE))
        {
			...
			m_lastDownScrollPos = 0; // liurundan
            // LUC
			rect.top = GetFixedRowHeight(bIncludeFreezedCells);
            //ScrollWindow(0, yScroll, rect);
	...
	case SB_PAGEDOWN:
		if (scrollPos < m_nVScrollMax)
		{
			...
			if ( scrollPos > m_lastDownScrollPos )
			{
				SetScrollPos32(SB_VERT, scrollPos);
				m_lastDownScrollPos = scrollPos;
				//   			m_ScrollBarWnd->m_sbVert.InvalidateScrollBar(); // liurundan
				// LUC
				rect.top = GetFixedRowHeight(bIncludeFreezedCells);
				InvalidateRect(rect);
			}
    ...
    default: m_lastDownScrollPos = 0; // liurundan
        break;
    }
}
...
COleDataSource* CGridCtrl::CopyTextFromGrid()
{
	...
	DWORD dwLen = (DWORD)(str.GetLength() + 1) * sizeof(TCHAR);
	sf.Write(str.GetBuffer(sizeof(TCHAR)), dwLen);
	str.ReleaseBuffer();
	HGLOBAL hMem = sf.Detach();
...
int CGridCtrl::GetScrollPos32(int nBar, BOOL bGetTrackPos /* = FALSE */)
{
	...
	if (bGetTrackPos)
	{
		if ( m_ScrollBarWnd )
		{
 			m_ScrollBarWnd->GetSkinScrollInfo(nBar,&si,SIF_TRACKPOS);
			return si.nTrackPos;
		}
		else
		{
	...
	else
	{
		if ( m_ScrollBarWnd )
		{
			m_ScrollBarWnd->GetSkinScrollInfo(nBar,&si,SIF_POS);
			return si.nPos;
 		}
		else
		{
...
BOOL CGridCtrl::SetScrollPos32(int nBar, int nPos, BOOL bRedraw /* = TRUE */)
{
	...
	SetScrollInfo(nBar, &si, FALSE); // liurundan

	if ( m_ScrollBarWnd )
	{
		SCROLLINFO scibar;
		scibar.cbSize = sizeof(SCROLLINFO);
		GetScrollInfo(nBar,&scibar,SIF_ALL);		
		 
		BOOL redraw = ( nPos == ( (nBar == SB_VERT) ? m_nVScrollMax : m_nHScrollMax) ) ? FALSE : bRedraw;
		m_ScrollBarWnd->SetSkinScrollInfo(nBar,&si,redraw,m_bShowVertSrollBarWindow);
...
void CGridCtrl::ResetScrollBars()
{
    ...
    if (rect.left >= rect.right || rect.top >= rect.bottom)
    {
		...
		m_bShowVertSrollBarWindow = FALSE; // liurundan
        return;
    }

 	ShowScrollBar(SB_VERT,SW_HIDE);
	if ( !m_bShowHorzSrollBar )
	{
    ...
    if (VisibleRect.Height() < VirtualRect.Height())
    {
		...
		m_bShowVertSrollBarWindow = TRUE;
    ...
    */
    ...
    SetScrollInfo(SB_HORZ, &si, FALSE); // liurundan true
	if ( m_ScrollBarWnd )
   		m_ScrollBarWnd->SetSkinScrollInfo(SB_HORZ, &si, TRUE, IsVisibleHScroll() );
 
    si.fMask |= SIF_DISABLENOSCROLL;
    si.nPage = (m_nVScrollMax>0)? VisibleRect.Height() : 0;
    si.nMin = 0;
    si.nMax = m_nVScrollMax;
    SetScrollInfo(SB_VERT, &si, FALSE);// liurundan true
	...
	if ( m_ScrollBarWnd )
	{
 		m_ScrollBarWnd->SetSkinScrollInfo(SB_VERT, &si, TRUE, m_bShowVertSrollBarWindow ); //IsVisibleVScroll()
 		m_lastDownScrollPos = 0;
	}
  
}

void CGridCtrl::SetShowSrollBar(UINT bar, BOOL bshow)
{
	switch(bar)
	{
	case SB_HORZ: m_bShowHorzSrollBar = bshow; break;
	case SB_VERT: m_bShowVertSrollBar = bshow; break;
	case SB_BOTH:
		m_bShowHorzSrollBar = bshow;
		m_bShowVertSrollBar = bshow;
		break;
	default: break;
	}
}
...
// liurundan
int CALLBACK CGridCtrl::pfnCustomCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);
	...
	std::string str1 = CStringA( pCell1->GetText() );
	std::string str2 = CStringA( pCell2->GetText() );
  
	return Ysbot::LogicCompare( const_cast<wchar_t *>(Ansi2Unicode(str1).c_str()), const_cast<wchar_t *>(Ansi2Unicode(str2).c_str()) );
}
...
// liurundan
void CGridCtrl::ExpandColumn( int nCol, int width, BOOL bExpandToFit )
{
	...
	////////////////////////////// ExpandColumnsToFit ////////////////////////////////////////////
	ExpandColumnsToFit(bExpandToFit);
	 
	return;

	BOOL bExpandFixed = bExpandToFit;
	if (bExpandFixed)
	{
...
void CGridCtrl::AutoSize(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
    ...
    {
        if( GetRowHeight( nRow) > 0 )
            (*m_pArrayRowHeights)[nRow] = 1; // liurundan
    }
...
// 	FlatSB_EnableScrollBar(this->m_hWnd, SB_VERT, ESB_DISABLE_BOTH); 
...
void CGridCtrl::EnsureVisible(int nRow, int nCol)
{
	...
	int right = nCol - maxVisibleCol;
	int left = nCol < m_nFixedCols + m_nFreezedCols ? 0 : minVisibleCol - nCol;
	...
	int up = nRow < m_nFixedRows + m_nFreezedRows ? 0 : minVisibleRow - nRow;

	int iColumnStart;
	int iRowStart;

	iColumnStart = maxVisibleCol + 1;
	while (right > 0)
	{
		if (GetColumnWidth(iColumnStart) > 0)
			SendMessage(WM_HSCROLL, SB_LINERIGHT, 0);
	...
	if (m_nScrollX < 0)
	{
		while (rectCell.left < GetFixedColumnWidth(TRUE) && IsVisibleHScroll()) // Junlin Xu: added to prevent infinite loop
		{
			SendMessage(WM_HSCROLL, SB_LINELEFT, 0);
			if (!GetCellRect(nRow, nCol, rectCell))
			{
...
// virtual liurundan
void CGridCtrl::OnEndEditCellInPlace( int nRow, int nCol, CString str, DWORD data )
{
	CString strCurrentText = GetItemText(nRow, nCol);
	DWORD newData = GetItemData(nRow,nCol);

	if (strCurrentText != str || newData != data )
	{
		SetItemText(nRow, nCol, str);
		if (ValidateEdit(nRow, nCol, str) && 
			SendMessageToParent(nRow, nCol, GVN_ENDLABELEDIT) >= 0)
...
void CGridCtrl::invalidate_sel_bottom()
{
    Invalidate();
	SendMessage(WM_VSCROLL, SB_BOTTOM, 0);
}
