...
BOOL CTreeGridCtrl::ShowRow(GTV_HTREENODE hItem, int nRow, BOOL bShow)
{
	...
	if(!bShow)
	{
		if (GetRowHeight(nRow) > 0) // new  add
		{
...
BOOL CTreeGridCtrl::SetTreeItemText( GTV_HTREENODE hItem, int nColumn, LPCTSTR lpszItem )
{
 	int nRow = FindRowIndex(hItem);
	return SetItemText(nRow,nColumn,lpszItem);
}

CString CTreeGridCtrl::GetTreeItemText( GTV_HTREENODE hItem, int nColumn )
{
	int nRow = FindRowIndex(hItem);
