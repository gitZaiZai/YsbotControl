...
#define HTL_BUTTON_IMG_WIDTH  24//9 liurundan +-
#define HTL_BUTTON_IMG_HEIGHT 24//9

#define HTL_HORZ_LINE_RIGHT_MARGIN 2

#define HTL_PLUS_SIGN_RADIUS 6//3
...
void CTreeCell::DrawTreeItemButton(CDC* pDC, CRect rcButtonArea)
{
	if(HasButton())
	{
		...
		// liurundan
		HBITMAP hBmpPlus  = m_pGridCell->GetGrid()->m_hBmpPlus;
		HBITMAP hBmpMinus = m_pGridCell->GetGrid()->m_hBmpMinus;
		if ( hBmpPlus && hBmpMinus )
		{
 			CDC bitmapDC;
		    ...
		    {
				BITMAP bm_minus;
				GetObject(hBmpPlus,sizeof(bm_minus),&bm_minus);

				hOldBmp =::SelectObject(bitmapDC,hBmpMinus);

				pDC->StretchBlt( rcSign.left,rcSign.top,rcSign.Width(), rcSign.Height(),
					&bitmapDC, 0, 0, bm_minus.bmWidth, bm_minus.bmHeight, 
					SRCCOPY );
		    }
			else
			{
				BITMAP bm_plus;
				GetObject(hBmpPlus,sizeof(bm_plus),&bm_plus);

 				hOldBmp=::SelectObject(bitmapDC,hBmpPlus);

				pDC->StretchBlt( rcSign.left,rcSign.top,rcSign.Width(), rcSign.Height(),
					&bitmapDC, 0, 0, bm_plus.bmWidth, bm_plus.bmHeight, 
					SRCCOPY );
			}
...
{	
	...
	else
	{
		...
		if(bDraw)
		{
			...
			// 绘制收弹按钮的间隔，当有些Item没有子项时，文字顶格显示。
			int  RowIndex = pGridCtrl->FindRowIndex(m_hTreeItem );
// 			if( m_nTreeItemLevel == 0 && m_hTreeItem->m_arChildNodes.GetSize() == 0 && RowIndex == 1 ) // liurundan
			if( !pGridCtrl->IsShownTreeNodeButton(m_hTreeItem) ) // liurundan
			{
