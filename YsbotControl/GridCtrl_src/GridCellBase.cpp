...
void CGridCellBase::operator=(const CGridCellBase& cell)
{
	...
	SetDrawSortArrow(cell.IsDrawSortArrow());
	OnCopy(cell);
}
...
// 			TextClr = ::GetSysColor(COLOR_HIGHLIGHTTEXT);			
			TextBkClr = pGrid->GetBkHighLightColour(); // liurundan
			TextClr = pGrid->GetTextHighLightColour();
            bEraseBkgnd = TRUE;
        }
...
//         pDC->SetTextColor(::GetSysColor(COLOR_HIGHLIGHTTEXT));
        ...
        rect.right++; rect.bottom++;    // FillRect doesn't draw RHS or bottom
        pDC->FillSolidRect(rect, pGrid->GetBkHighLightColour()); // liurundan
        rect.right--; rect.bottom--;
        pDC->SetTextColor(pGrid->GetTextHighLightColour());
    ...
    if (pGrid->GetImageList() && GetImage() >= 0)
    {
        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
        {
            ...
            */
			...
			CPoint centerRect = rect.CenterPoint();
			
			centerRect.Offset(-nImageWidth/2,-nImageHeight/2);

			BOOL showImageCenter = GetFormat() & SS_CENTERIMAGE;

            if ((nImageWidth + rect.left <= rect.right) && (nImageHeight + rect.top <= rect.bottom))    
			{
                pGrid->GetImageList()->Draw(pDC, GetImage(), showImageCenter ? centerRect : rect.TopLeft(), ILD_NORMAL); // liurundan
			}
    ...
    // Draw sort arrow
    if ( m_bDrawSortArrow && pGrid->GetSortColumn() == nCol && nRow == 0)
    {
