...
// 		MAKEINTRESOURCE(IDB_TREEBTNS)));
	m_imgBtns.Create (16, 16, ILC_COLOR32|ILC_MASK,2,1);
	CBitmap* pBmpBtns = CBitmap::FromHandle(LoadBitmap(AfxGetInstanceHandle(),
		MAKEINTRESOURCE(IDB_TREEBTNS2))); // liurundan
	ASSERT(pBmpBtns);
...
// 					ILD_TRANSPARENT);
				m_imgBtns.Draw(pDC, nImg, CPoint(x-iconSize.cx/2-14,yCenterItem-8), 
					ILD_TRANSPARENT); // liurundan
			}
...
void CColumnTreeCtrl::Initialize()
{
	...
	m_Tree.SetFont(&font_);        // 设置树形控件字体
	m_Header.SetFont(&font_);
	m_Header2.SetFont(&font_);
