...
//   			pDCMem->FillRect(rc,&br); // liurundan
// 			CBrush brush;
			CPen pen;
// 			brush.CreateSolidBrush(RGB(102,102,102));
			//pen.CreatePen(PS_SOLID,0,RGB(255,255,255));
			COLORREF cr = GetBkColor();
			COLORREF cr2 = RGB(220,220,220);
			pen.CreatePen(PS_SOLID,0,cr2);
			CBrush *pOldBrush=pDCMem->SelectObject(&br);
	...
	if ( m_icon )
	{
 		DrawTheIcon(pDCMem,&rc);
	}

	if ( m_csBitmaps )
	{
		DrawTheBitmap(pDCMem,&rc);
	}
...
// 		dwFlags = DT_CENTER; // liurundan
	...
	else
	{
		...
		if ( m_icon )
		{
			textRect.OffsetRect(dwWidth + 5,0);
		}
	 
		pDCMem->DrawText( strText,textRect, dwFlags );
...
// 	if ( m_csBitmaps )
// 	{
// 		DeleteObject(m_csBitmaps);
// 		m_csBitmaps = NULL;
//  	}
}

// void CLabel::DrawTheBitmap(CDC* pDC, RECT* rpItem)
// {
// 	HDC			hdcBmpMem	= NULL;
...
// 
// 	hbmOldBmp = (HBITMAP)::SelectObject( hdcBmpMem, m_csBitmaps );
// 
// 	hdcMem = ::CreateCompatibleDC(NULL);
// 
// // 	hbmT = (HBITMAP)::SelectObject( hdcMem, m_hMaskBitmaps );
//  	 
// // 	::BitBlt(pDC->m_hDC, rImage.left, rImage.top, dwWidth, dwHeight, hdcMem, 0, 0, SRCAND);
// 
// 	::BitBlt(pDC->m_hDC, rImage.left, rImage.top, dwWidth, dwHeight, hdcBmpMem, 0, 0, SRCPAINT);
//   
// 
...
void CLabel::DrawTheBitmap( CDC* pDC, RECT* rpItem)
{
	...
	CRect (rc);
	GetClientRect(&rc);
	CDC* pcdc_ = this->GetDC();
	CMemDC MemDC(*pcdc_, rc);
	CDC& dc = MemDC.GetDC();
 
 	hbmOldBmp = (HBITMAP)::SelectObject( dc.m_hDC, m_csBitmaps );

	if ( !use_stretch_drawbmp )
	{
		pDC->BitBlt(rImage.left,rImage.top,rImage.Width(),rImage.Height(),&dc,0,0,SRCCOPY); 
 	}
	else
		pDC->StretchBlt(rImage.left,rImage.top,rImage.Width(),rImage.Height(),&dc,0,0,dwWidth,dwHeight,SRCCOPY); 

	::SelectObject( dc.m_hDC, hbmOldBmp );

	ReleaseDC(pcdc_);
  	 
} // End of DrawTheBitmap

void CLabel::DrawTheIcon( CDC* pDC, RECT* rpItem )
{
 	CRect imgRect(rpItem);
	CSize imgsize( dwWidth, dwHeight ); 
	int hoffset = ( imgRect.Height() - dwHeight ) / 2;
	if ( hoffset < 1 )
 		hoffset = 1;

  	imgRect.OffsetRect(1,hoffset);
	// Ole'!
	BOOL res = pDC->DrawState( imgRect.TopLeft(),
		imgsize, 
		m_icon,
		DSS_NORMAL, 
		(CBrush*)NULL);
...
//////////////////////////////////////////////////////////////////////////
CLabel& CLabel::SetText(const CString& strText,BOOL redraw)
{
...
//////////////////////////////////////////////////////////////////////////
// NOTES:	liurundan add 20170330		
//
// Name     Date        Version Comments
//     		02072002    1.6     Origin
//////////////////////////////////////////////////////////////////////////
DWORD CLabel::SetLabelIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn)
{
	if( IsWindow(this->GetSafeHwnd()) == FALSE ) 
	{
 		return -1;
	}
	nIDIcom = nIconIn;
	...
	hIconIn = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIconIn), IMAGE_ICON, nCxDesiredIn, nCyDesiredIn, 0);
 	...
 	return SetLabelIcon(hIconIn);
} // End of SetIcon

DWORD CLabel::SetLabelIcon( HICON hIconIn )
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
DWORD CLabel::SetLabelBitmaps( CString strBmpName )
{
	if( IsWindow(this->GetSafeHwnd()) == FALSE ) 
		return -1;
	...
	// Load bitmap In
	hBitmapIn = (HBITMAP)::LoadImage(::AfxGetInstanceHandle(), strBmpName,	IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
    if ( hBitmapIn == NULL )
 		return -1;

 	return SetLabelBitmaps(hBitmapIn, RGB(0,0,0) );
}

DWORD CLabel::SetLabelBitmaps(int nBitmapIn, COLORREF crTransColorIn)
{
	if( IsWindow(this->GetSafeHwnd()) == FALSE ) 
 		return -1;
	...
	return SetLabelBitmaps(hBitmapIn, crTransColorIn);
} // End of SetBitmaps

DWORD CLabel::SetLabelBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
	...
	if (hBitmapIn)
	{
		...
		dwWidth  = (DWORD)csBitmapSize.bmWidth;
		dwHeight = (DWORD)csBitmapSize.bmHeight;
  
		// Create mask for bitmap In
// 		m_hMaskBitmaps = CreateBitmapMask(hBitmapIn, dwWidth, dwHeight, crTransColorIn);
		if (m_hMaskBitmaps == NULL)
		{
// 			return -1;
		} // if
 	} // if
...
HBITMAP CLabel::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
	...
	SetTextColor(crSaveDestText); // liurundan
...
void CLabel::FreeResources(BOOL bCheckForNULL)
{
	if ( bCheckForNULL )
	{
		// Destroy icons
		...
		// Destroy bitmaps
		if (m_csBitmaps)	::DeleteObject(m_csBitmaps);

		// Destroy mask bitmaps
		if (m_hMaskBitmaps)	::DeleteObject(m_hMaskBitmaps);
 	}
...
void CLabel::DrawTheText()
{
	...
	CDC& dc = MemDC.GetDC();
  	...
  	COLORREF crText = dc.SetTextColor(m_crText);
	...
	CPen pen;
	//pen.CreatePen(PS_SOLID,0,RGB(255,255,255)); liurundan
	pen.CreatePen(PS_SOLID,0,RGB(220,216,220));
	...
	UINT nMode = dc.SetBkMode(TRANSPARENT);
 	dc.DrawText( m_text, textRect, m_dwTextStyle );
	pcdc_->BitBlt(rc.left,rc.top,rc.Width(), rc.Height(), &dc,0,0,SRCCOPY);
...
HBRUSH CLabel::CtlColor(CDC* pDC/*pDC*/, UINT /*nCtlColor*/)
{
