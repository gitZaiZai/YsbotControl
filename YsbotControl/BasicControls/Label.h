...
public:
	...
	virtual CLabel& SetText(const CString& strText, BOOL redraw = TRUE);
	...
	virtual DWORD SetLabelIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn);
	virtual DWORD SetLabelIcon(HICON hIconIn);
	virtual DWORD SetLabelBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn);
	virtual DWORD SetLabelBitmaps(int nBitmapIn, COLORREF crTransColorIn);
	virtual DWORD SetLabelBitmaps(CString strBmpName);
	virtual int GetIconID() { return nIDIcom; }

	const COLORREF& GetBkColor() { return m_crLoColor; }

	void set_stretch_drawbmp (bool b) { use_stretch_drawbmp = b; }
// Attributes
public:
protected:
	void UpdateSurface();
	...
	void DrawTheBitmap(CDC* pDC, RECT* rpItem);
	void DrawTheIcon( CDC* pDC, RECT* rpItem );
	void DrawTheText();
	...
	CString         m_text;
	HBITMAP         m_csBitmaps;
	HBITMAP         m_hMaskBitmaps;
	DWORD dwWidth;
	DWORD dwHeight;
	DWORD m_dwTextStyle;
    int nIDIcom;
	bool use_stretch_drawbmp;
