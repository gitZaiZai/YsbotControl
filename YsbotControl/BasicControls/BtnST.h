...
public:
    ...
    enum	{	BTNST_PRESSED_LEFTRIGHT = 0,	// Pressed style from left to right (as usual)
				...
				BTNST_PRESSED_NONE,			    // Pressed style none
			};

    enum	{	MENU_CENTERALIGN	= 0,		 
				MENU_RIGHTBUTTON,				 
				MENU_LEFTALIGN,			 
 			};
...
public:
	...
	void  SetBtBackGroudColor(COLORREF cr);
	void  SetBtTextColor(COLORREF cr);
	void  SetBtTextAlign(int align) { m_align_type = align; }
	...
	DWORD ChangeBorderStyle(BOOL bChange = TRUE, BOOL bRepaint = TRUE); // liurundan
	...
	void SizeToContent();
	void SetTextOffset(int xoffset) { m_textOffset = xoffset; }
	...
	void  SetMenuAlignMode( BYTE alignMode, BOOL bRepaint = TRUE ); // edit by lrd
    void  SetButtonDeleted( BOOL b ) { m_bDeleted = b; }
	BOOL  IsMenuDisplayed() { return m_bMenuDisplayed; }
...
protected:
	...
	BYTE        m_menuAlign;        // edit by lrd
    BOOL        m_bChangeBorderStyle;
	BOOL        m_bDeleted;
