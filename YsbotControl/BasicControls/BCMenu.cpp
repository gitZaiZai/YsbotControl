...
BCMenu::BCMenu()
{
	...
	m_iconY = 25;            // ...
 	...
 	m_customFont.CreateFont(16, 8, 0, 0, FW_NORMAL,
		FALSE, FALSE, 0, 
		ANSI_CHARSET,
		OUT_DEFAULT_PRECIS, 
		CLIP_DEFAULT_PRECIS,
...
void BCMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	...
	if( m_bNoIconBar )   // edit by lrd dan.
 		barwidth = 0;
 	// remove the selected bit if it's grayed out
	if(lpDIS->itemState & ODS_GRAYED&&!xp_select_disabled){
		...
		if (m_buseCustomFont) // edit by lrd dan.
 			pFont = pDC->SelectObject(&m_customFont);
 		else
	...
	if(state & MF_SEPARATOR){
		...
		BOOL checkflag=FALSE;
		COLORREF crText = GetSysColor(COLOR_MENUTEXT);
		COLORREF crSelect = RGB(49,69,182); // GetSysColor(COLOR_HIGHLIGHT); lrd edit
		COLORREF crSelectFill;
// 		if(!IsWinXPLuna())crSelectFill=LightenColor(crSelect,0.85);
// 		else crSelectFill=LightenColor(crSelect,0.7);
		if(!IsWinXPLuna())crSelectFill=LightenColor(crSelect,0.0 ); // lrd edit
		else crSelectFill = LightenColor(crSelect,0.0); // lrd edit
		CBrush m_brSelect;
		CPen m_penBack;
		int x0,y0,dx,dy;
		int nIconNormal=-1,xoffset=-1,global_offset=-1;
		...
		if(state&ODS_SELECTED){ // draw the down edges
			...
			pDC->SelectObject (pOldPen);
			crText = GetSysColor(COLOR_HIGHLIGHTTEXT);  // lrd edit
...
// 			pDC->Draw3dRect (rect2,m_newclrBack,m_newclrBack);
			...
			pDC->Draw3dRect (rect2,RGB(0,0,255),RGB(0,0,255));
		}
		
		// draw the text if there is any
		//We have to paint the text only if the image is nonexistant
		if (m_buseCustomFont) // edit by lrd
			pFont = pDC->SelectObject(&m_customFont);
