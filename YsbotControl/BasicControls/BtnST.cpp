...
CButtonST::CButtonST()
{
	...
	m_bDrawBorder = TRUE; 
    m_bChangeBorderStyle = FALSE;
	// By default icon is aligned horizontally
	m_byAlign = ST_ALIGN_HORIZ; 

	m_menuAlign = MENU_CENTERALIGN;
    m_bDeleted  = FALSE;
...
{	
	...
	else
	{
...
#endif
		{
			...
			switch(m_menuAlign)
			{
			case MENU_CENTERALIGN: 
				dwRetValue = psub->TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | 
									   TPM_NONOTIFY | TPM_RETURNCMD, rWnd.left, rWnd.bottom, this, NULL);
				break;
			case MENU_LEFTALIGN:
				dwRetValue = psub->TrackPopupMenu(TPM_RIGHTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | 
					TPM_NONOTIFY | TPM_RETURNCMD, rWnd.left, rWnd.top, this, NULL);
				break;
			case MENU_RIGHTBUTTON:
				dwRetValue = psub->TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | 
					TPM_NONOTIFY | TPM_RETURNCMD, rWnd.right, rWnd.bottom, this, NULL);
				break;
			default: break;
			}
 #else
...
void CButtonST::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if ( m_align_type == 1 ) // center
	{
	    rpCaption->OffsetRect((centerRect.Width() - rpCaption->Width())/2 - m_textOffset, (centerRect.Height() - rpCaption->Height())/2); // liurundan
 	}
	else if ( m_align_type == 0 ) // right
	{
		rpCaption->OffsetRect(8, (centerRect.Height() - rpCaption->Height())/2); // liurundan
	}
// 	
...
void  CButtonST::SetMenuAlignMode( BYTE alignMode, BOOL bRepaint ) // edit by lrd
{
    m_menuAlign = alignMode;
	if (bRepaint)	Invalidate();
}
...
DWORD CButtonST::ChangeBorderStyle(BOOL bChange /* = TRUE */, BOOL bRepaint /* = TRUE */)
{
	m_bChangeBorderStyle = bChange;
	if (bRepaint) Invalidate();
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	...
	CRect rectoffset = *pRect;
	if ( m_ptPressedOffset.x == 0 &&
		 m_ptPressedOffset.y == 0 )
	{
		rectoffset.left += 1;
		rectoffset.top  += 1;
	...
	pDC->FillRect(&rectoffset, &brBackground);
...
DWORD CButtonST::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	...
	if (m_bIsPressed)
	{
		if (m_bIsFlat)
		{
			if ( m_bDrawBorder )
			{
				if ( m_ptPressedOffset.x == 0 && m_ptPressedOffset.y == 0 ) // Pressed style none
				{
				  ;
				}
				else
	...
	{
		CPen penBtnHiLight(PS_SOLID, 2, GetSysColor(COLOR_BTNHILIGHT)); // White
		CPen pen3DLight(PS_SOLID, 0, GetSysColor(COLOR_3DLIGHT));       // Light gray
		CPen penBtnShadow(PS_SOLID, 0, GetSysColor(COLOR_BTNSHADOW));   // Dark gray
		CPen pen3DDKShadow(PS_SOLID, 0, GetSysColor(COLOR_3DDKSHADOW)); // Black
		CPen penBlue(PS_SOLID, 0, RGB(0,0,255)); // Blue
		COLORREF bkcr;
		GetColor(CButtonST::BTNST_COLOR_BK_IN,&bkcr);
		CPen penBg(PS_SOLID,2,bkcr);
		if (m_bIsFlat)
		{
			if ( m_bChangeBorderStyle )
			{
				CPen* pOldPen = pDC->SelectObject(&penBtnHiLight);
				pDC->MoveTo(pRect->left, pRect->bottom); // edited by rundan
...
void CButtonST::SetBtBackGroudColor(COLORREF cr)
{
	SetColor(CButtonST::BTNST_COLOR_BK_FOCUS, cr );
	SetColor(CButtonST::BTNST_COLOR_BK_OUT, cr);
	SetColor(CButtonST::BTNST_COLOR_BK_IN, cr);
}

void CButtonST::SetBtTextColor(COLORREF cr)
{
 	SetColor(CButtonST::BTNST_COLOR_FG_OUT, cr);
	SetColor(CButtonST::BTNST_COLOR_FG_IN, cr);
	SetColor(CButtonST::BTNST_COLOR_FG_FOCUS, cr);
}