...
#include "..\ScintillaLib\include\SciLexer.h"
#include "..\ScintillaLib\include\Scintilla.h"
...
public:
	...
	int SciCall(unsigned int msg, uptr_t wParam=0, sptr_t lParam=0, bool threadcall = false ) {
		switch (msg) {
		...
		if ( threadcall )
		{
			retVal = Send(msg,wParam,lParam);
		}
		else
		{
			retVal= fn(ptr, msg, wParam, lParam);
	...
	int CallPointer(unsigned int msg, uptr_t wParam, void *s) {
		return SciCall(msg, wParam, reinterpret_cast<sptr_t>(s));
	}
