...
Editor::Editor() {
	...
	scrollWidth = 2000; // liurundan 2000
...
void Editor::SetSelection(SelectionPosition currentPos_, SelectionPosition anchor_) {
	...
	//printf(" SetSelection [%d %d]\n",sel.RangeMain().Start().Position(),sel.RangeMain().End().Position() ); // liurundan
}
...
void Editor::PaintSelMargin(Surface *surfWindow, PRectangle &rc) {
	...
	marginView.PaintMargin(surface, topLine, rc, rcMargin, *this, vs); // liurundan
...
void Editor::NotifyLButtonUp(int start, int end) 
{
 	SCNotification scn = {};
 
	scn.nmhdr.code = SCN_LBUTTONUP;
	scn.x = start;
	scn.y = end;
	NotifyParent(scn);
}
...
void Editor::ButtonUp(Point pt, unsigned int curTime, bool ctrl) {
	...
	if (HaveMouseCapture()) 
	{
		...
		if (inDragDrop == ddDragging) {
			...
			if (selectionType == selChar) {
				if (sel.Count() > 1) {
					...
					//printf(" newPos btup sel [%d %d]\n",newPos.Position(), sel.RangeMain().anchor.Position() );
					//NotifyLButtonUp(sel.RangeMain().anchor.Position(), newPos.Position());
				}
			}
			sel.CommitTentative();
		}
		...
		{
			NotifyLButtonUp( SelectionStart().Position(), SelectionEnd().Position() );
 		}
	}
