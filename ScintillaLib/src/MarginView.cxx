	...
	for (size_t margin = 0; margin < vs.ms.size(); margin++) {
		if (vs.ms[margin].width > 0) {
			...
			while ((visibleLine < model.cs.LinesDisplayed()) && yposScreen < rc.bottom) {
				...
				int marks = model.pdoc->GetMark(lineDoc); // liurundan
				if (!firstSubLine)
					marks = 0;
				...
				if (vs.ms[margin].style == SC_MARGIN_NUMBER) {
					if (firstSubLine) {
						...
						rcNumber.left = xpos;
						if ( marks == 0 ) // have arrow, don't draw line number.
						{
							DrawTextNoClipPhase(surface, rcNumber, vs.styles[STYLE_LINENUMBER],
								rcNumber.top + vs.maxAscent, number, static_cast<int>(strlen(number)), drawAll); // liurundan draw line nuber.
						}
