...
class ScintillaWin :
	...
	void NotifyScrollBar(SCNotification scn); // liurundan
...
{
	SCNotification scn = {};
	scn.nmhdr.code = SCN_SET_SCROLLBAR;
	scn.ch = nBar;
...
// 	si = *lpsi;
	...
	scn.wParam  = (uptr_t)(lpsi);
	scn.updated = (int) bRedraw;
 
	int nRet = ::SetScrollInfo(MainHWND(), nBar, lpsi, bRedraw);
...
{
	SCROLLINFO scrollInfo = *lpsi;
	SCNotification scn = {};
	scn.nmhdr.code = SCN_GET_SCROLLBAR;
	scn.ch = nBar;
 	scn.wParam  = (uptr_t)(&scrollInfo);
	//NotifyScrollBar(scn);
 	*lpsi = scrollInfo;
	//return true;
...
bool ScintillaWin::ModifyScrollBars(int nMax, int nPage) {
	...
	if ((sci.nMin != 0) ||
		...
		sci.fMask = SIF_PAGE | SIF_RANGE;
		sci.nMin = 0;
		sci.nMax = horizEndPreferred; // liurundan horbar
		sci.nPage = pageWidth;
		sci.nPos = 0;
		sci.nTrackPos = 1;
...
// 	              GetCtrlID(), reinterpret_cast<LPARAM>(&scn));
	::SendMessage(MainHWND(), WM_NOTIFY,
	              GetCtrlID(), reinterpret_cast<LPARAM>(&scn));
}

// liurundan
void ScintillaWin::NotifyScrollBar(SCNotification scn) 
{
	scn.nmhdr.hwndFrom = MainHWND();
	scn.nmhdr.idFrom = GetCtrlID();
	::SendMessage( MainHWND(), WM_NOTIFY,GetCtrlID(), reinterpret_cast<LPARAM>(&scn));
}
...
void ScintillaWin::ScrollMessage(WPARAM wParam) {
	...
	int spos  = HiWord((unsigned long)wParam);
	int scode = LoWord((unsigned long)wParam);
	// 	printf("ScrollInfo scode = %d, spos = %d\n",scode, spos );
...
void ScintillaWin::HorizontalScrollMessage(WPARAM wParam) {
	int spos  = HiWord((unsigned long)wParam); // liurundan
	int scode = LoWord((unsigned long)wParam);
	...
	case SB_LINEDOWN:	// May move past the logical end
		...
		{	// Hit the end exactly
			xPos = scrollWidth - static_cast<int>(rcText.Width())+2;
		}
		break;
	case SB_PAGEUP:
		xPos -= pageWidth;
...
sptr_t ScintillaWin::DirectFunction(
    sptr_t ptr, UINT iMessage, uptr_t wParam, sptr_t lParam) {
// 	DWORD currID = ::GetCurrentThreadId(); // 示教盒串口读取线程访问编辑窗口时，报错，ID不等。
// 	DWORD winID  = ::GetWindowThreadProcessId(reinterpret_cast<ScintillaWin *>(ptr)->MainHWND(), NULL);
